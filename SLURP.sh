...
cat "$basename"_minimum100bp.fq | seqkit grep -s -i -P -R 1:100 -m 2 -p GCTCTATCTTCTTT -o "$basename"_q1m2.fq
...
cat "$basename"_minimum100bp.fq | seqkit grep -s -i -P -R 1:100 -m 2 -p CTGATATTGCTGGG -o "$basename"_q3m2.fq
...
cat "$basename"_minimum100bp.fq | seqkit grep -s -i -P -R -100:-1 -m 2 -p CCCAGCAATATCAG -o "$basename"_q3m2rc.fq
...
cat "$basename"_q1m2_q3m2rc.fq "$basename"_rcq3m2.fq | seqkit rmdup -o "$basename"_SLURP_stranded.fq
wc -l "$basename"_SLURP_stranded.fq | awk '{print $1 * 0.25, "reads found in '$basename'_SLURP_stranded.fq", $1*0.25/'$parent_reads'*100, "% of parent"}'
